...
class ExpertLvl1(GlobalCreatureBaseClass):
    """
    Medium humanoid (any race) creature - ExpertLvl1
    Source: AiDeDd Monster Database (D&D 5e SRD)
    """
    
    # Valores por defecto extraídos del scraping
    DEFAULT_STATS = {'hit_points': 11, 'min_level': 1, 'level': 1, 'STR': 10, 'DEX': 15, 'CON': 12, 'INT': 13, 'WIS': 10, 'CHAR': 14, 'armor_class': 14, 'alignment': '- Armor Class  14 (studded leather) Hit Points  11 (2d8 + 2) Speed  30 ft. STR 10 (+0) DEX 15 (+2) CON 12 (+1) INT 13 (+1) WIS 10 (+0) CHA 14 (+2) Saving Throws  Dex +4 Skills  Acrobatics +4', 'legendary': False, 'size': 'Medium', 'type': 'humanoid (any race)', 'hit_points_up': [1, 1, 1], 'STR_up': [1, 1, 0], 'DEX_up': [0, 1, 0], 'CON_up': [0, 0, 1], 'INT_up': [1, 0, 0], 'WIS_up': [0, 0, 1], 'CHAR_up': [0, 0, 0], 'abilities': ['helpful', 'shortsword', 'dagger', 'shortbow']}

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)

    def helpful(self) -> str:
        """The expert can take the Help action as a bonus action.Tools. The expert has thieves' tools and a musical instrument."""
        return "The expert can take the Help action as a bonus action.Tools. The expert has thieves' tools and a musical instrument."
