# bases1/young-dragon-turtle.py
"""
YoungDragonTurtle creature class scraped from https://www.aidedd.org/dnd/monstres.php?vo=young-dragon-turtle
"""
from creature_base import GlobalCreatureBaseClass


class YoungDragonTurtle(GlobalCreatureBaseClass):
    """
    Huge Dragon creature - YoungDragonTurtle
    Source: AiDeDd Monster Database (D&D 5e SRD)
    """
    
    # Valores por defecto extraídos del scraping
    DEFAULT_STATS = {'hit_points': 178, 'min_level': 1, 'level': 1, 'STR': 21, 'DEX': 10, 'CON': 19, 'INT': 10, 'WIS': 12, 'CHAR': 12, 'armor_class': 19, 'alignment': 'typically Neutral Armor Class  19 (natural armor) Hit Points  178 (17d12 + 68) Speed  20 ft.', 'legendary': False, 'size': 'Huge', 'type': 'Dragon', 'hit_points_up': [1, 1, 1], 'STR_up': [1, 1, 0], 'DEX_up': [0, 1, 0], 'CON_up': [0, 0, 1], 'INT_up': [1, 0, 0], 'WIS_up': [0, 0, 1], 'CHAR_up': [0, 0, 0], 'abilities': []}

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)

